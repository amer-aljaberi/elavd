        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-6 lg:gap-4">
          ...
          <div className="flex flex-wrap items-center justify-center lg:justify-end gap-5 sm:gap-8 lg:gap-10 w-full lg:w-auto">
            ...
            <div className="flex items-center gap-3 text-[13px] text-muted-foreground font-medium">
              <span className="font-cairo uppercase tracking-wider text-[11px] text-muted-foreground">{t('Show')} :</span>
                ...
                {showOptions.map((opt) => (
                   <button
                    ...
                    className={`hover:text-primary cursor-pointer transition-colors ${
                      searchParams.get('limit') === opt.toString() ? 'text-primary font-bold scale-110' : 'text-muted-foreground'
                    }`}
                  >
                    {opt}
                  </button>
                    ...
                    <div className="absolute top-full mt-2 ltr:right-0 rtl:left-0 bg-background shadow-2xl rounded-xl border border-border p-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 min-w-[180px]">
                        {options.map((sort) => (
                          <button 
                             ...
                             className={`w-full cursor-pointer text-start px-4 py-2 hover:bg-muted/30 rounded-lg text-xs font-semibold font-cairo transition-colors ${
                               currentSort === sort.val ? 'text-primary bg-primary/5' : 'text-muted-foreground hover:text-primary'
                             }`}
                          >
                             {t(sort.key)}
                          </button>
